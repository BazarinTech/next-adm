...
if (isset($_POST['submit'])) {
    ...
    $insert = $query->insert('admins', ['name' => $name, 'username' => $username, 'role' => $role, 'permissions' => $permissions]);
}
?>
<!DOCTYPE html>
                                ...
                                <div x-show="open" x-transition x-transition.duration.300 class="relative w-full max-w-lg p-0 my-8 overflow-hidden bg-white border rounded-lg border-slate-200 dark:bg-darklight dark:border-darkborder">
                                    ...
                                    <div class="p-5 space-y-4">
                                        <div class="space-y-1">
                                            ...
                                            <input type="text" name="name" class="form-input h-14" placeholder="eg Christof Bitech" required>
                                        </div>
                                        ...
                                        <template x-for="item in paginatedItems" :key="item.id">
                                            <tr x-show="showElement" x-data="{ showElement: true }">
                                                ...
                                                <td>
                                                    ...
                                                    " x-show="!item.editing"></span>
                                                    <input type="text" class="form-input" x-ref="role" x-model="item.role" x-on:keydown.enter="item.editing = false; updater(item.id, 'role', item.role);" x-show="item.editing">
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <button class="text-danger ltr:ml-2 rtl:mr-2" x-on:click="showElement = false; deleteItem('admins', item.id)">
