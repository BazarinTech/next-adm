...
if (isset($_POST['submit'])) {
    ...
    $insert = $query->insert('rolls', ['name' => $name, 'cost' => $price, 'rolls' => $rolls]);
            ...
            <div class="h-[calc(100vh-60px)] relative overflow-y-auto overflow-x-hidden p-4 space-y-4 detached-content">
                <nav class="w-full flex justify-between items-center mb-4">
                    <ul class="space-y-2 detached-breadcrumb">
                        ...
                        <li class="text-xl font-semibold text-slate-800 dark:text-slate-100">Rolls</li>
                    </ul>
                    <div x-data="modals">
                        ...
                        <form action="rolls" method="POST" enctype="multipart/form-data" class="fixed inset-0 bg-black/80 z-[99999] hidden overflow-y-auto dark:bg-dark/90" :class="open && '!block'">
                            <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                <div x-show="open" x-transition x-transition.duration.300 class="relative w-full max-w-lg p-0 my-8 overflow-hidden bg-white border rounded-lg border-slate-200 dark:bg-darklight dark:border-darkborder">
                                    <div class="flex items-center justify-between px-5 py-3 bg-white border-b border-slate-200 dark:bg-darklight dark:border-darkborder">
                                        <h5 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Add Rolls</h5>
                                    ...
                                    <div class="p-5 space-y-4">
                                        ...
                                        <div class="space-y-1 my-4">
                                            <label>Rolls</label>
                                            <input name="rolls" type="number" class="form-input h-14" placeholder="rolls" required>
                                        </div>
                                        <div class="space-y-1 my-4">
                                            <label>Roll image</label>
                ...
                <div class="flex flex-col gap-4 min-h-[calc(100vh-212px)]">
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <div class="card">
                        <h2 class="mb-4 text-base font-semibold capitalize text-slate-800 dark:text-slate-100">Rolls Records</h2>
                        
                            <div class="overflow-auto" x-data="{ items: <?= htmlspecialchars(json_encode($rolls_records), ENT_QUOTES, 'UTF-8') ?>,
                                                          searchTerm: '',
                            currentPage: 1,
                            itemsPerPage: 3,
                            ...
                            get filteredItems() {
                                ...
                                return this.items.filter(item => 
                                    ...
                                    item.rolls.toLowerCase().includes(searchLower) || 
                                        ...
                                        <template x-for="item in paginatedItems" :key="item.id">
                                            <tr x-show="showElement" x-data="{ showElement: true }">
                                                ...
                                                <td>
                                                    ...
                                                    " x-show="!item.editing"></span>
                                                    <input type="number" class="form-input" x-ref="price" x-model="item.price" x-on:keydown.enter="item.editing = false; updater(item.id, 'cost', item.price);" x-show="item.editing">
                                                </td>
                                                <td>
                                                    <span x-text="item.rolls" x-on:dblclick="
                                                        item.editing = <?= $isEdit ?>;
                                                        $nextTick(() => $refs.rolls.focus());
                                                    " x-show="!item.editing"></span>
                                                    <input type="number" class="form-input" x-ref="rolls" x-model="item.rolls" x-on:keydown.enter="item.editing = false; updater(item.id, 'rolls', item.rolls);" x-show="item.editing">
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <button class="text-danger ltr:ml-2 rtl:mr-2" x-on:click="showElement = false; deleteItem('rolls', item.id)">
...
function updater(id, field, value) {
    fetch('actions/update_rolls.php', {
        method: 'POST',
